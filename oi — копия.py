...
class BybitOITracker:
    def __init__(self, api_key: str, api_secret: str, telegram_token: str):
        ...
        self.last_alert_messages: Dict[str, Dict[int, int]] = {}  # {symbol: {chat_id: message_id}}
        self.last_reset = datetime.now().date()
        self.sent_alerts: Dict[str, float] = {}  # {symbol: timestamp} - для предотвращения дублей
    ...
    def calculate_oi_change(self, symbol: str, current_oi: float, current_time: datetime, time_window: int) -> Dict:
        ...
        if len(historical_data) < 2:  # Нужно минимум 2 точки для сравнения
            return None
    ...
    def get_alert_number(self, symbol: str) -> int:
        ...
        if today != self.last_reset:
            ...
            self.sent_alerts.clear()  # Очищаем историю отправленных алертов
        ...
        # Проверяем, не было ли уведомления в последние 5 минут (защита от дублей)
        if symbol in self.sent_alerts:
            time_since_last = time.time() - self.sent_alerts[symbol]
    ...
    def register_alert(self, symbol: str, alert_data: Dict):
        ...
        self.sent_alerts[symbol] = time.time()  # Запоминаем время отправки
...
"""
    ...
    async def send_alert_to_users(self, alert_message: str, symbol: str):
        ...
        for chat_id in self.chat_ids:
            ...
            if settings["enabled"]:
                try:
                    ...
                    if symbol in self.last_alert_messages:
                        reply_to_message_id = self.last_alert_messages[symbol].get(chat_id)
                    ...
                    if symbol not in self.last_alert_messages:
                        self.last_alert_messages[symbol] = {}
                    self.last_alert_messages[symbol][chat_id] = sent_message.message_id
                    
                except Exception as e:
                    print(f"Ошибка отправки сообщения пользователю {chat_id}: {e}")
                    # Продолжаем отправлять другим пользователям даже если один фейлится
            ...
            if oi_result and price_result:
                ...
                if len(self.oi_history[symbol]) > 1 and len(self.price_history[symbol]) > 1:
                    oi_change_data = self.calculate_oi_change(
                        ...
                        oi_result["datetime"],
                        15  # используем фиксированное время для проверки
                    )
                    ...
                    if oi_change_data and abs(oi_change_data["change_percent"]) >= 2.0:  # Минимальный порог 2%
                        for chat_id in self.chat_ids:
                            settings = self.get_user_settings(chat_id)
                            ...
                            if self.should_alert(oi_change_data["change_percent"], settings["oi_threshold_percent"]):
                                ...
                                if alert_number > 0:
                                    ...
                                    await self.send_alert_to_users(alert_message, symbol)
                                    self.register_alert(symbol, oi_change_data)
                                    alerts_triggered += 1
                                    break  # Выходим из цикла по пользователям - отправляем только раз
                                else:
...
"""
        ...
        try:
            ...
            print(f"Ошибка отправки сообщения в start_command: {e}")
    ...
    async def settings_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            print(f"Ошибка отправки сообщения в settings_command: {e}")
...
"""
        ...
        try:
            ...
            print(f"Ошибка отправки сообщения в status_command: {e}")
    ...
    async def stop_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            print(f"Ошибка отправки сообщения в stop_command: {e}")
    ...
    async def button_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "close":
            ...
            try:
                ...
                print(f"Ошибка в button_callback (threshold): {e}")
            ...
            try:
                ...
                print(f"Ошибка в button_callback (time): {e}")
            ...
            try:
                ...
                print(f"Ошибка в button_callback (coins): {e}")
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if waiting_for == "threshold":
            try:
                ...
                print(f"Ошибка в handle_message (threshold): {e}")
            ...
            try:
                ...
                print(f"Ошибка в handle_message (time): {e}")
            context.user_data.pop("waiting_for", None)
            ...
            if text.lower() == "все":
                ...
                try:
                    ...
                    print(f"Ошибка в handle_message (coins all): {e}")
            else:
                ...
                try:
                    ...
                    print(f"Ошибка в handle_message (coins list): {e}")
            context.user_data.pop("waiting_for", None)
